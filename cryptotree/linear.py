...
def arrays_to_ptx(arrays: List[np.ndarray], encoder: seal.CKKSEncoder, scale: float) -> List[seal.Plaintext]:
    ...
    outputs = []
    for array in arrays:
        ptx = encoder.encode(DoubleVector(list(array)), scale)
        outputs.append(ptx)
    return outputs
...
def dot_product_plain(ctx: seal.Ciphertext, ptx: seal.Plaintext,
    ...
    output = evaluator.multiply_plain(ctx, ptx, output)
...
def test_dot_product_plain(x: List[float], y: List[float],
    ...
    ptx = encoder.encode(x, scale, ptx)

    ctx = encryptor.encrypt(ptx, ctx)

    pty = encoder.encode(y, scale, pty)
