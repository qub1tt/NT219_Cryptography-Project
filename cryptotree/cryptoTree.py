...
class HomomorphicTreeEvaluator:
    ...
    def compare(self, ctx: seal.Ciphertext) -> seal.Ciphertext:
        ...
        output = self.evaluator.add_plain(ctx, self.b0_ptx)
    ...
    def match(self, ctx: seal.Ciphertext) -> seal.Ciphertext:
        ...
        output = self.evaluator.add_plain(output, self.b1_ptx)
    ...
    def reduce(self, outputs: List[seal.Ciphertext]):
        ...
        for i, output in enumerate(outputs):
            ...
            mask_ptx = self.encoder.encode(mask, self.scale)
            self.evaluator.mod_switch_to_inplace(mask_ptx, output.parms_id())

            if i == 0:
                scores = self.evaluator.multiply_plain(output, mask_ptx)
                self.evaluator.rescale_to_next_inplace(scores)
                scores.scale(self.scale)
            else:
                temp = self.evaluator.multiply_plain(output, mask_ptx)
    ...
    def to_ptx(self, array):
        ...
        ptx = self.encoder.encode(array, self.scale)
        return ptx
...
class HomomorphicTreeFeaturizer:
    ...
    def encrypt(self, x: np.ndarray):
        ...
        ptx = self.encoder.encode(features, self.scale)
